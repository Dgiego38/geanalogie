...
var REF_LINE_REG = /^0 (?:(@[_a-zA-Z0-9]*@) )?(?<type>_[_A-Z][_a-zA-Z]{2,}|_?[A-Z][a-zA-Z]{2,}) ?(?<value>.*)/gm;
var LINE_REG = /(?<indent>^[0-9]) (?:(@[_a-zA-Z0-9]*@) )?(?<type>_[_A-Z][_a-zA-Z]{2,}|_?[A-Z][a-zA-Z]{2,}) ?(?<value>.*)/;
var ID_REG = /^@[_a-zA-Z0-9]+@/;
var ID_GETTER_REG = /^(?<at>@)?(?<letter>[A-Z])?/i;
var ID_SPLIT_REG = /^@[_a-zA-Z0-9]+@:/;
var MAX_FILE_SIZE_TO_SYNC = 1024 * 1024 * 20;
  ...
  orderBy(orders, ClassName = _List) {
    ...
    const sortedItems = this.entries().toSorted(
      ([itemAId, itemA], [itemBId, itemB]) => {
        ...
        return sortValue2 * (direction === "DESC" ? -1 : 1);
      }
    );
  ...
  if (showMarriages || isOnStage) {
    (indi?.get("FAMS")?.toValueList().values() ?? []).forEach((fam) => {
      ...
      if (isOnStage && famRef) {
        ...
        } else if (famRef.WIFE && indi && famRef.WIFE.value === indi.id) {
...
var PlaceType = /* @__PURE__ */ ((PlaceType2) => {
  PlaceType2["All"] = "ALL";
  PlaceType2["Birth"] = "BIRT";
  PlaceType2["Marriage"] = "MARR";
  PlaceType2["Death"] = "DEAT";
  PlaceType2["Events"] = "EVEN";
  PlaceType2["Military"] = "_MILT";
  PlaceType2["MilitaryId"] = "_MILTID";
  return PlaceType2;
})(PlaceType || {});
...
// src/classes/indi.ts
var Existed = /* @__PURE__ */ ((Existed2) => {
  Existed2["SPOUSE"] = "spouse";
  Existed2["YES"] = "yes";
  return Existed2;
})(Existed || {});
var CustomTags = /* @__PURE__ */ ((CustomTags2) => {
  CustomTags2["UnknownGivenname"] = "Unknown givenname";
  CustomTags2["UnknownSurname"] = "Unknown surname";
  CustomTags2["UnattachedMember"] = "Unattached member";
  CustomTags2["IgnoredMember"] = "Ignored member";
  CustomTags2["UnknownAncestor"] = "Unknown Ancestor";
  return CustomTags2;
})(CustomTags || {});
var ALLOWED_FACTS = [
  "EVEN",
...
var isIntersectedRange = (range1, range2) => {
...
var splitRange = (to, by) => {
  ...
  results.push({
    range: make(
      Number.isFinite(overlapStart) ? overlapStart : void 0,
      Number.isFinite(overlapEnd) ? overlapEnd : void 0
    ),
    to: true,
    by: true
  });
...
function parseRangeBounds(rangeKey) {
  const split = rangeKey.split("-");
  const start = split[0] ? Number(split[0]) : void 0;
  const end = split[1] ? Number(split[1]) : void 0;
  return [start, end];
}
function isRangeContained(containedRange, containerRange) {
  ...
  return containedStartNum >= containerStartNum && containedEndNum <= containerEndNum;
}
function extractSplitPoints(ranges) {
  const points = /* @__PURE__ */ new Set();
  ranges.forEach((range) => {
    const [start, end] = parseRangeBounds(range);
    if (start !== void 0) points.add(start);
    if (end !== void 0) points.add(end + 1);
  });
  return Array.from(points).sort((a, b) => a - b);
}
function generateSplitRanges(splitPoints) {
  const ranges = [];
...
function splitOverlappingRanges(rangesToValues) {
  ...
  if (splitPoints.length > 0) {
    ...
    const hasRangeBeforeFirst = ranges.some((range) => {
      ...
      return start === void 0 || start < firstPoint;
    });
    if (hasRangeBeforeFirst) {
      splitRanges.push(fromTuple(void 0, firstPoint - 1));
    }
  }
  for (let i = 0; i < splitPoints.length - 1; i++) {
    const start = splitPoints[i];
  ...
  if (splitPoints.length > 0) {
    ...
    const hasRangeAfterLast = ranges.some((range) => {
      ...
      return end === void 0 || end >= lastPoint;
    });
    if (hasRangeAfterLast) {
      splitRanges.push(fromTuple(lastPoint, void 0));
    }
  }
  return splitRanges.map((splitRange2) => {
    const matchingValues = [];
    rangesToValues.forEach(([originalRange, value]) => {
      if (isRangeContained(splitRange2, originalRange)) {
        matchingValues.push(value);
      }
    });
    return [splitRange2, matchingValues];
  }).filter(([, values]) => values.length > 0);
}
function findMatchingRangeForSplitRange(splitRange2, rangesToValues) {
  const matchingValues = [];
  rangesToValues.forEach(([originalRange, value]) => {
    if (isRangeContained(splitRange2, originalRange)) {
      matchingValues.push(value);
...
function extractSeparationYears(childName, ranges, parentName, ensureArray) {
  ...
  Object.entries(ranges).forEach(([rangeKey, entries]) => {
    ...
    pts.forEach((pt) => {
      ...
      if (keyDiffersFromTown && pointsToParent && isSingleParent) {
        ...
        const split = rangeKey.split("-");
        const start = split[0] ? Number(split[0]) : void 0;
        if (typeof start === "number" && !separationYears.includes(start)) {
...
var alphabets = chars.reduce((acc, curr, idx) => {
  acc[curr] = idx;
  return acc;
}, {});
var convertAlphabet = (value) => {
  return value.toLowerCase().replace(/(dzs|cs|dz|gy|ly|ny|sz|ty|zs|.)/gi, (_, m) => {
    const code = alphabets[m];
    return code !== void 0 ? `${`${code}`.padStart(`${chars.length}`.length, "0")}` : m;
  });
};
var sortValue = (valueA, valueB) => {
  return convertAlphabet(valueA).localeCompare(convertAlphabet(valueB));
};
var getNameDesc = (nameOrder = "first-last") => (itemA, _keyA, itemB, _keyB) => {
  const valueA = nameOrder === "first-last" ? [itemA.NAME?.GIVN || "", itemA.NAME?.SURN || ""] : [itemA.NAME?.SURN || "", itemA.NAME?.GIVN || ""];
  const valueB = nameOrder === "first-last" ? [itemB.NAME?.GIVN || "", itemB.NAME?.SURN || ""] : [itemB.NAME?.SURN || "", itemB.NAME?.GIVN || ""];
  return -sortValue(
    valueA.filter(Boolean).join(" "),
    valueB.filter(Boolean).join(" ")
  );
};
var getNameAsc = (nameOrder = "first-last") => (itemA, _keyA, itemB, _keyB) => {
  const valueA = nameOrder === "first-last" ? [itemA.NAME?.GIVN || "", itemA.NAME?.SURN || ""] : [itemA.NAME?.SURN || "", itemA.NAME?.GIVN || ""];
  const valueB = nameOrder === "first-last" ? [itemB.NAME?.GIVN || "", itemB.NAME?.SURN || ""] : [itemB.NAME?.SURN || "", itemB.NAME?.GIVN || ""];
  return sortValue(
    valueA.filter(Boolean).join(" "),
...
var getNameAscAndBirth = (nameOrder = "first-last") => (itemA, keyA, itemB, keyB) => {
  const nameSort = getNameAsc(nameOrder)(itemA, keyA, itemB, keyB);
  if (nameSort !== 0) {
    return nameSort;
  }
  return getBirthAsc(itemA, keyA, itemB);
};
...
var getMarriageAscAndBirth = (person) => (itemA, keyA, itemB, keyB) => {
...
var createFam = (gedcom, id, main, parent) => {
  return createProxy(
...
var getName = () => package_default.name;

// src/classes/gedcom.ts
var isObjectPatch = (value) => {
  return !!value && typeof value === "object" && !Array.isArray(value);
};
var GedCom = class extends Common {
  constructor() {
    super();
    this.tagMembers = {};
    this.reflist = {};
  ...
  createNodeFromPatchValue(value, main) {
    if (value === void 0 || value === null) {
      return void 0;
    }
    const node = createCommon(this, void 0, main);
    if (Array.isArray(value)) {
      value.forEach((item) => {
        const childNode = this.createNodeFromPatchValue(item, node);
        if (childNode) {
          node.assign("DATA", childNode);
        }
      });
      return node;
    }
  ...
  applyObjectToCommon(target, patch) {
    Object.entries(patch).forEach(([rawTag, value]) => {
      if (rawTag === "value") {
        if (value === null || value === void 0 || Array.isArray(value) || isObjectPatch(value)) {
          return;
        }
      ...
      if (isObjectPatch(value)) {
        ...
        const objectNode = existing ?? createCommon(this, void 0, target.main ?? this);
  ...
  cloneIndis(target, source, avoidKeys = [], removeFromOriginalList = true) {
    ...
    const spousesFamily = hasFAMS ? sourceIndi.FAMS?.toList() : void 0;
    ...
    if (newCloned.id) {
      ...
      if (childrenFamily?.length) {
        childrenFamily.forEach((fam) => {
          ...
          const newChild = createCommon(this, void 0, fam.ref);
      ...
      if (spousesFamily?.length) {
        spousesFamily.forEach((fam) => {
          ...
          const newSpouse = createCommon(this, void 0, fam.ref);
      ...
      if (removeFromOriginalList && newCloned.type && newCloned.type !== targetIndi.type) {
        this.getList(`@@${newCloned.type}`)?.removeItem(
          newCloned.id
        );
      }
      newCloned.type = "INDI";
  ...
  mergeIndis(target, source, removeFromOriginalList = true) {
    ...
    if (removeFromOriginalList && sourceIndi?.type && sourceIndi.id) {
      this.getList(`@@${sourceIndi.type}`)?.removeItem(
        sourceIndi.id
      );
    }
  }
  ...
  fromList(id) {
    return id ? this.reflist?.[id] : void 0;
  }
  tagByName(name) {
  ...
  getIndiRelatedLists(indis) {
    ...
    const individuals = this.indis()?.filter((indi, indiKey) => {
      if (indis.includes(indiKey)) {
        indi.getRefs()?.values()?.forEach((ref) => {
    ...
    const usedLists = Object.entries(this).filter(([prop, list]) => {
      return prop !== "@@INDI" && prop.startsWith("@@") && list instanceof List;
    });
    const lists = {};
    if (individuals) {
  ...
  hasTag(tag) {
    const tagName = typeof tag === "string" ? tag : tag?.get("NAME")?.toValue();
    if (!tagName) {
      return false;
    }
  ...
  hasUnknownAncestor() {
    return !!this.tagMembers?.["Unknown Ancestor" /* UnknownAncestor */]?.indis?.length;
  }
  hasIgnoredMember() {
    return !!this.tagMembers?.["Ignored member" /* IgnoredMember */]?.indis?.length;
  }
  hasUnattachedMember() {
    return !!this.tagMembers?.["Unattached member" /* UnattachedMember */]?.indis?.length;
  }
  hasUnknownGivenname() {
    return !!this.tagMembers?.["Unknown givenname" /* UnknownGivenname */]?.indis?.length;
  }
  hasUnknownSurname() {
    return !!this.tagMembers?.["Unknown surname" /* UnknownSurname */]?.indis?.length;
  }
  hasNonRelevant() {
  ...
  getAllPlacesWithIndis(usedIndis = []) {
    ...
    const allPlaces = {};
    indis?.forEach((indi) => {
      if (!indi.id || usedIndis.length && !usedIndis.includes(indi.id)) {
        return;
      }
      const places = indi.getPlaces();
      places.forEach((placeObj) => {
  ...
  stats(individuals) {
    ...
    if (individuals) {
      ...
      families = this.fams()?.filter(
        (fam) => fam.id ? familyIds.has(fam.id) : false
      );
    } else {
      families = this.fams();
    }
    ...
    if (firstBirthDate || firstDeathDate) {
      const isBirth = !firstBirthDate || firstDeathDate && firstDeathDate < firstBirthDate ? false : true;
      firstPersonEvent = {
    ...
    if (lastBirthDate || lastDeathDate) {
      const isBirth = !lastDeathDate || lastBirthDate && lastDeathDate < lastBirthDate ? true : false;
      lastPersonEvent = {
        type: isBirth ? "BIRT" : "DEAT",
...
var isGedcomString = (gedcomString) => {
  return typeof gedcomString === "string" && gedcomString[0] === "0" && gedcomString.split("\n").slice(0, 1e3).every((l) => !l || /^\d+\s+/.test(l));
};
var validateGedcomContent = (content) => {
  if (!content) {
  ...
  /[\x00-\x08\x0E-\x1F]/.test(firstBytes);
  if (isBinary) {
    return {
      valid: false,
...
var MERGE_ID_INCREMENT_MULTIPLIER = 1e3;
var valueToString = (value) => {
  if (!value) return "";
  return value.toString?.() || String(value?.toValue?.() || "");
};
var mergeGedcoms = (targetGedcom, sourceGedcom, strategy = "id") => {
  ...
  const famMatchMap = /* @__PURE__ */ new Map();
  sourceFams?.forEach((sourceFam) => {
    ...
    const finalHusbId = sourceHusbId ? idMap.get(sourceHusbId) : void 0;
    const finalWifeId = sourceWifeId ? idMap.get(sourceWifeId) : void 0;
    const finalChildIds = sourceChildIds.map((id) => idMap.get(id)).filter(Boolean);
    let matchedTargetFam;
    targetFams?.forEach((targetFam) => {
      if (matchedTargetFam) return;
      ...
      if (husbMatch && wifeMatch) {
        ...
        const totalUniqueChildren = (/* @__PURE__ */ new Set([
          ...finalChildIds,
          ...targetChildIds
        ])).size;
        const hasSpouses = finalHusbId || finalWifeId || targetHusbId || targetWifeId;
        const hasMatchingChildren = totalUniqueChildren > 0 && childOverlap / totalUniqueChildren >= 0.5;
  ...
  const clonedIndis = /* @__PURE__ */ new Map();
  sourceIndis?.forEach((sourceIndi) => {
    if (!sourceIndi.id) return;
  ...
  const clonedFams = /* @__PURE__ */ new Map();
  sourceFams?.forEach((sourceFam) => {
    if (!sourceFam.id) return;
...
// src/classes/objes.ts
var Objects = class _Objects extends List {
  copy() {
    const newList = new _Objects();
...
// src/classes/repos.ts
var Repositories = class _Repositories extends List {
  copy() {
    const newList = new _Repositories();
    this.entries().forEach(([key, value]) => {
      newList.item(key, value);
    });
    return newList;
...
// src/classes/sours.ts
var Sources = class _Sources extends List {
  copy() {
    const newList = new _Sources();
    this.entries().forEach(([key, value]) => {
      newList.item(key, value);
    });
    return newList;
...
// src/classes/subms.ts
var Submitters = class _Submitters extends List {
  copy() {
    const newList = new _Submitters();
    this.entries().forEach(([key, value]) => {
      newList.item(key, value);
    });
    return newList;
...
// src/classes/note.ts
var CommonNote = class extends Common {
  constructor(gedcom, id, main, parent) {
    super(gedcom, id, main, parent);
    delete this.id;
  }
  ...
  get value() {
    const contents = this.get("CONT")?.toList().map((content) => content.value).join("\n");
    if (contents) {
      return `${this._value}
${contents}`;
    }
...
var createCommonNote = (gedcom, id, main, parent) => {
  return createProxy(
...
var Obje = class extends Common {
  standardizeMedia(namespace, override = true, urlGetter) {
    ...
    const title = this?.get("FILE.TITL")?.toValue() ?? this?.get("TITL")?.toValue() ?? "";
    const note = this?.get("FILE.NOTE")?.toValue() ?? this?.get("NOTE")?.toValue() ?? "";
    ...
    const newUrl = createCommon(this._gedcom, void 0, this.main);
    newUrl.value = url;
...
var createObje = (gedcom, id, main, parent) => {
  return createProxy(
...
// src/classes/repo.ts
var Repo = class extends Common {
};
var createRepo = (gedcom, id, main, parent) => {
  return createProxy(
...
// src/classes/sour.ts
var Sour = class extends Common {
};
var createSour = (gedcom, id, main, parent) => {
  return createProxy(
...
// src/classes/subm.ts
var Subm = class extends Common {
};
var createSubm = (gedcom, id, main, parent) => {
  return createProxy(
    new Subm(gedcom, id, main, parent)
  );
};

// src/utils/common-creator.ts
var create = (gedcom, type, id, nodes) => {
  ...
  if (id) {
    ...
    if (type === "REPO") {
      prevNode = createRepo(gedcom, void 0, mainNode, curNode);
    } else if (type === "SUBM") {
      prevNode = createSubm(gedcom, void 0, mainNode, curNode);
    } else if (type === "SOUR") {
      prevNode = createSour(gedcom, void 0, mainNode, curNode);
    } else if (type === "OBJE") {
      prevNode = createObje(gedcom, void 0, mainNode, curNode);
    } else if (type === "DATE") {
      prevNode = createCommonDate(gedcom, void 0, mainNode, curNode);
    } else if (type === "NOTE") {
      prevNode = createCommonNote(gedcom, void 0, mainNode, curNode);
    } else if (type === "NAME" && mainNode instanceof Indi) {
      prevNode = createCommonName(gedcom, void 0, mainNode, curNode);
    } else {
      prevNode = createCommon(gedcom, void 0, mainNode, curNode);
    }
  }
  prevNode.type = type;
...
var GedcomTree = {
  ...
  parseHierarchy: function(content, options) {
    ...
    const refs = refMatch && Array.isArray(refMatch) ? refMatch.reduce((acc, curr) => {
...
`.split(/\r?\n/).reduce((acc, line) => {
    ...
    if (!linesJoined.includes("1 _IS_PURGED true") && getRawSize(linesJoined) > MAX_FILE_SIZE_TO_SYNC) {
      linesJoined = linesJoined.replace(
        /1\s(NOTE|OBJE|SOUR)([^\n]*\n)([23456789]\s.+\n)*/gm,
        ""
      ).replace(/0\s(@[^@]+@)(\sSOUR[^\n]*\n)([123456789]\s.+\n)*/, "").replace(/0 HEAD(.*)\n/, "0 HEAD$1\n1 _IS_PURGED true\n");
    ...
    lines.forEach((line, index) => {
      ...
      try {
        ...
        const id = idCheck ? `@${idCheck}@` : void 0;
        ...
        if (prevNode instanceof Common) {
          ...
          if (value) {
            ...
            if (isId(value) && refs[value]) {
              ...
              if (linkingKey && linkingKey === type && curNode instanceof Indi && curNode?.id) {
    ...
    if (tags.length) {
      gedcom.tagMembers = tags.reduce((acc, tag) => {
        const tagName = tag?.get("NAME")?.toValue();
        if (!tagName || !tag?.ref || !tag?.main || !(tag.main instanceof Indi)) {
          return acc;
        }
        if (!acc[tagName]) {
          acc[tagName] = {
    ...
    if (linkingKey) {
      links.forEach((linkingIndi, _linkingId) => {
        ...
        const linked = linking?.get(linkingKey)?.ref;
        if (!linking) {
          return;
        }
